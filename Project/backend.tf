...
module "s3_backend" {
  ...
  bucket_name = "my-bucket" # Унікальна назва бакету
  # Назва таблиці DynamoDB для блокування стану Terraform
  dynamodb_table_name = "my-devops-terraform-state-lock"
  # Теги для ресурсів
  tags = {
    Environment = "Dev"
    Project     = "FlexibleRDS"
...
terraform {
  backend "s3" {
    bucket         = "my-devops-terraform-state-bucket-12345bucketraform.tfstate"
